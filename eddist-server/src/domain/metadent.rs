...
pub fn generate_meta_ident(asn: u32, ip_addr: &str, ua: &str, seed: u32) -> String {
    ...
    while bb_chars.len() < 2 {
        bb_chars.push('0');
    }

    let bb = bb_chars.into_iter().collect::<String>();
...
mod tests {
    ...
    fn test_meta_ident_format() {
        ...
        for (ua, ip, asn) in test_cases {
            ...
            assert_eq!(ident.len(), 9, "Identifier '{}' should be 9 characters long", ident);
            assert!(ident.contains('-'), "Identifier '{}' should contain hyphen", ident);

            let parts: Vec<&str> = ident.split('-').collect();
            assert_eq!(parts.len(), 2, "Identifier '{}' should have exactly 2 parts", ident);
            assert_eq!(parts[0].len(), 4, "First part '{}' should be 4 characters", parts[0]);
            assert_eq!(parts[1].len(), 4, "Second part '{}' should be 4 characters", parts[1]);

            // Verify all characters are alphanumeric
            for c in ident.chars() {
                assert!(c.is_ascii_alphanumeric() || c == '-', "Character '{}' should be alphanumeric or hyphen", c);
            }
        }
    }
}
